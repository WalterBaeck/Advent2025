...
if (1)  putchar('\n');
    ...
    //////////////////// Examine all possible partitions ////////////////////
    for (;;)
    {
      //////////////////// Descent ////////////////////
      for (++Order; Order<NrOfLights; Order++)
      {
        // Reset every Partition at downstream Orders
          ...
          else  /* Index=0 */
          {
            ...
            for (OtherLightNr=0; OtherLightNr<NrOfLights; OtherLightNr++)
              if (Button[ButtonNr][OtherLightNr])
                Total[OtherLightNr] -= Push[ButtonNr];
/* Debug */
if (1)  printf("  ^ Exhausted Partition Order #%d resetting Button=%d from Push=%d\n",
Order, ButtonNr, Push[ButtonNr]);
PrintDebug();
            Push[ButtonNr] = 0;
      ...
      for (OtherLightNr=0; OtherLightNr<NrOfLights; OtherLightNr++)
        if (Button[ButtonNr][OtherLightNr])
          Total[OtherLightNr]++;
/* Debug */
if (1)  printf("  + Order #%d increasing Button=%d to Push=%d\n",
Order, ButtonNr, Push[ButtonNr]);
PrintDebug();
      ...
      // As a nonzero choice was found someplace within this Partition,
      //   change should definitely have been possible without breaking constraints
      if (Needed < 0)
      {
        fprintf(stderr, "  + Unchangeable: Order #%d Light #%d has %d leftover at end\n",
            Order, LightNr, Needed);
        exit(4);
      }
      Partition[Order][--Index] = Needed;
      ButtonNr = ButtonAtOrder[Order][Index];
      ...
      for (OtherLightNr=0; OtherLightNr<NrOfLights; OtherLightNr++)
        if (Button[ButtonNr][OtherLightNr])
          Total[OtherLightNr] += Needed;
/* Debug */
if (1)  printf("  + Part[#%d][%d] last Button=%d set to Push=%d\n",
Order, Index, ButtonNr, Push[ButtonNr]);
PrintDebug();
